...
<%
	...
	HashMap<String, FesCcobro> mapaAlumnosEnLinea	= (HashMap<String,FesCcobro>) request.getAttribute("mapaAlumnosEnLinea");
...
<div class="container-fluid">
	<h2>Tuition Breakdown <small class="text-muted fs-4">( <%=facultadNombre%> )</small></h2>
	<form name="frmProceso" action="calculos" method="post">
	...
	<div class="alert alert-info">
		...
		<a href="calculos?CargaId=<%=cargaId%>&FacultadId=<%=facultadId%>&Bloques=<%=bloques%>" class="btn btn-primary" type="submit"><i class="fas fa-sync-alt"></i></a>    
	</div>	
	</form>
...
<% 		
	int row=0;
	for(CargaAlumno carga : lisCalculos){
		row++;
		...
		String fecha = "";
		if(mapaAlumnosEnLinea.containsKey(carga.getCodigoPersonal()+carga.getCargaId()+carga.getBloqueId())){
			fecha = mapaAlumnosEnLinea.get(carga.getCodigoPersonal()+carga.getCargaId()+carga.getBloqueId()).getFecha();
		}
