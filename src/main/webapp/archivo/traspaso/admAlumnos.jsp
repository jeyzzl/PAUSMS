...
<script>
	function Traspasar(){
		if (confirm("¿ Deseas traspasar todos los datos ?")){
			document.location.href= "admAlumnos?Accion=1";
		}
	}
	function subir(){
		if (confirm("¿ Deseas subir todos los datos ?")){
			document.location.href= "admAlumnos?Accion=2";
		}
	}
</script>
<%
	...
	if (!error && accion.equals("1")){		
		COMANDO = "SELECT COUNT(*) AS TOTAL FROM ADM_DOCALUM";
		rset2 = stmt2.executeQuery(COMANDO);
		if (rset2.next()){			 	
			total = rset2.getInt(1);
		}
		
		COMANDO = "SELECT FOLIO,DOCUMENTO_ID,HOJA FROM ADM_DOCALUM";
		rset2 = stmt2.executeQuery(COMANDO);
		while (rset2.next()){
			...
			String comando = "INSERT INTO ADM_DOCALUM(FOLIO, DOCUMENTO_ID, HOJA)"
				+" VALUES(TO_NUMBER("+folio+",'99999999'),TO_NUMBER("+documentoId+",'99'),'"+hoja+"')";
	...
	if (!error2 && accion.equals("2")){
		...
		String path = application.getRealPath("archivo/traspaso/admDocAlum");
	    ...
	    for (File fileEntry : folder.listFiles()) {
	        if (fileEntry.isDirectory()) {
				...
				String hoja			= name[2];
				String arreglo[] 	= hoja.split("\\.");
				hoja				= arreglo[0];
				
				//System.out.println(fileEntry.getName()+" Folio:"+folio+" Documento:"+documentoId+" Hoja:"+hoja);				
				...
				*/
				...
				PreparedStatement ps = conn3.prepareStatement("UPDATE ADM_DOCALUM SET IMAGEN=?"+
					" WHERE FOLIO = TO_NUMBER("+folio+",'99999999') AND DOCUMENTO_ID = TO_NUMBER("+documentoId+",'99') AND HOJA = "+hoja);
				ps.setLong(1,oid);
		...
		mensaje = "Se traspasaron "+numRow+" renglones a la tabla de ADM_DOCALUM";
	}
	
%>
<div class="container-fluid">	
	<h1>Admision Doc. Alumnos(Documentos/archivos)</h1>
