...
<%
	...
	try{
        ...
        for(int i = 0; i < listCursos.size(); i++){
    		...
    		//tituloCurso.setTotalWidth(500f);
			...
			//tituloCurso.writeSelectedRows(0, -1, 200, 575, pdf.getDirectContent());
			...
			for(int j = 0; j < listUnidades.size(); j++){
				...
				for(int k = 0; k < listProductos.size(); k++){
					...
					if(!tipo.equals(mapaNuevoProducto.getTipo())){
						...
						celda = new PdfPCell(new Phrase(tipoNombre
								, FontFactory.getFont(FontFactory.HELVETICA, 8, Font.NORMAL, new BaseColor(r,g,b))));
						celda.setHorizontalAlignment(Element.ALIGN_LEFT);
						celda.setBorder(Rectangle.NO_BORDER);
						celda.setPadding(1f);
					...
					*/
					...
					celda = new PdfPCell(new Phrase(mapaNuevoProducto.getDescripcion()
							, FontFactory.getFont(FontFactory.HELVETICA, 9, Font.NORMAL, new BaseColor(r,g,b))));
					celda.setHorizontalAlignment(Element.ALIGN_LEFT);
					celda.setBorder(Rectangle.NO_BORDER);
					celda.setPadding(0f);
