...
<%	
	...
	List<CatFacultad> listaFacultades 		= (List<CatFacultad>)request.getAttribute("listaFacultades");
...
<div class="container-fluid">
	<h2>Notas máximas por facultad <small class="text-muted fs-4">(<%=cargaId%>)</small></h2>
	<div class="alert alert-info">
		<a href="maxCarga" style="text-align:center;" class="btn btn-primary"><i class="fas fa-arrow-left"></i></a>&nbsp;
	</div>
	<br>
...
<%
	...
	for(CatFacultad facultad : listaFacultades){
		...
		String baja = "0";
		if(mapaNotaBaja.containsKey(facultad.getFacultadId())){
 			baja = mapaNotaBaja.get(facultad.getFacultadId());
			//baja = String.valueOf(mapaNotaBaja.get(facultad.getFacultadId()));
		}
		
		String media = "0";
		if(mapaNotaMedia.containsKey(facultad.getFacultadId())){
			media = mapaNotaMedia.get(facultad.getFacultadId());
		//	media = String.valueOf(mapaNotaMedia.get(facultad.getFacultadId()));
		}
			
		String buena = "0";
		if(mapaNotaBuena.containsKey(facultad.getFacultadId())){
			buena = mapaNotaBuena.get(facultad.getFacultadId());
			//buena = String.valueOf(mapaNotaBuena.get(facultad.getFacultadId()));
		}
		
		
		String alta = "0";
		if(mapaNotaAlta.containsKey(facultad.getFacultadId())){
			alta = mapaNotaAlta.get(facultad.getFacultadId());
			//alta = String.valueOf(mapaNotaAlta.get(facultad.getFacultadId()));
		}
...
%>
	<tr>
		<td><%=row %></td>
		<td><a href="maxCarreras?CargaId=<%=cargaId%>&FacultadId=<%=facultad.getFacultadId()%>" class="btn btn-info btn-sm"><%=facultad.getNombreCorto()%></a></td>
		<td><%=facultad.getTitulo()+" de "+facultad.getNombreFacultad() %></td>
