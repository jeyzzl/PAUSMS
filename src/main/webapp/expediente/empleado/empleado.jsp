...
<div class="container-fluid">
	...
	<div class="alert alert-info">
		...
		<a href="editar" class="btn btn-primary"><i class="fas fa-plus"></i> Document</a>
	</div>	
	<div class="row">
		<div class="col-6">				
...
%>
					<a href="editar?DocumentoId=<%=exp.getDocumentoId()%>&Hoja=<%=i%>"><span class="badge <%=colorHoja%> rounded-pill"><%=i%></span></a>	
<% 			}
		}%>
			...
			<table class="table table-sm table-bordered">
				<thead class="table-info">
					<tr>
						...
						View 
				<%	if (existeHoja){%>
						<span class="text-muted">( <%=documentoNombre%> Page:<%=expEmpleado.getHoja()%>&nbsp;&nbsp;
						<a href="editar?DocumentoId=<%=expEmpleado.getDocumentoId()%>&Hoja=<%=expEmpleado.getHoja()%>"><i class="fas fa-edit"></i></a> 
						<a href="javascript:Borrar('<%=expEmpleado.getDocumentoId()%>','<%=expEmpleado.getHoja()%>')"><i class="fas fa-trash-alt"></i></a>
						)</span>
				<%	} %>
						</th>
					</tr>
