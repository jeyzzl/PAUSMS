...
<% 
	    ...
	    case 2: { // Grabar y modificar
	    	...
	    	if (CompetenciaU.existeReg(conEnoc, cursoCargaId, request.getParameter("Competencia").toUpperCase()) == false){
				if (CompetenciaU.insertReg(conEnoc, GrupoCompetencia)){
					Resultado = "Saved: " +GrupoCompetencia.getCursoCargaId();
				}else{
					Resultado = "Error saving: "+GrupoCompetencia.getCursoCargaId();
				}
			}else{	
				if (CompetenciaU.updateReg(conEnoc, GrupoCompetencia)){ 
					Resultado = "Strategy:"+GrupoCompetencia.getCursoCargaId()+" updated";
				}else{
					Resultado = "Error updating: "+GrupoCompetencia.getCursoCargaId();
				}
			}
			break;	
		...
		case 3:{//borrar
			...
			if (CompetenciaU.existeReg(conEnoc, cursoCargaId, request.getParameter("Competencia")) == true){
				if (CompetenciaU.deleteReg(conEnoc, cursoCargaId, request.getParameter("Competencia"))){
					Resultado = "Deleted: "+GrupoCompetencia.getCursoCargaId();
				}else{
					Resultado = "Error deleting: "+GrupoCompetencia.getCursoCargaId();
				}	
			}else{
					Resultado = "Not found: "+GrupoCompetencia.getCursoCargaId();
			}
			break;
}	
	 ...
	 ArrayList<aca.carga.CargaGrupoCompetencia> lisCompetencias  = CompetenciaU.getListAll(conEnoc, "ORDER BY COMPETENCIA_ID");
%>
<body>
<table style="width:100%">
   <tr>
     <td colspan="2" align="center" style="font-size:12pt"><strong>Competencies</strong></td>
   </tr>
   <tr>
...
<form name="frmAlta" method="post" action="competencia?CursoCargaId=<%=cursoCargaId%>&maestro=<%=maestro%>&materia=<%=materia%>">
  ...
  <table style="margin:0 auto; width:50%"> 
    ...
    <tr><td>&nbsp;</td></tr> 
    <tr>
      <td style="background-color: #5FB404;font-size: 10pt; color:white;" align="center" colspan="2" align="center" width="50%"><b>Partial Competencies</b></td>     
    </tr>
     <tr><td>&nbsp;</td></tr> 
    <tr>
...
<table style="margin: 0 auto; width:50%"> 
  ...
  <tr>
    ...
    <td width="10%" style="background-color: #D0F5A9;font-weight: bold; font-size:10pt" align="center">Item</td>
...
%>    	
  <tr>
     <td align="center"> <a href="competencia?Accion=1&CursoCargaId=<%= cursoCargaId%>&Competencia=<%= competencia.getCompetenciaId()%>"> 
       <img title="Edit" src="../../imagenes/editar.gif" alt="Modificar" ></a>
