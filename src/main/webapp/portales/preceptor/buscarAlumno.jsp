...
<script type="text/javascript">
	...
	function SubirCodigo( CodigoPersonal ,nombre){
	  		opener.forma.codigoPersonal.value=CodigoPersonal;
	  		opener.guardar('<%=cuarto%>');
	  		window.close();
	}	
</script>
<%
	...
	switch (nAccion){
		...
		case 2:{
			...
			if (AlumUtil.existeReg(conEnoc, request.getParameter("CodigoPersonal")) == true){
				alumno = AlumUtil.mapeaRegId(conEnoc, request.getParameter("CodigoPersonal"));
				sResultado = "Click on the student's name";
			}else{
...
<form name="frmnombre" method="POST" action="buscarAlumno?Accion=1&Origen=<%=origen%>&carpeta=<%=sCarpeta%>">
<input name="cuarto" type="hidden" value='<%=cuarto%>'>
...
<form name="frmcodigo" method="POST" action="buscarAlumno?Accion=2&Origen=<%=origen%>&carpeta=<%=sCarpeta%>">
<input name="Accion" type="hidden">
<input name="cuarto" type="hidden" value='<%=cuarto%>'>
  <tr align='CENTER'> 		
    <td> Student ID: 
...
<%				if(lisAlumno.size()==1){
		...
		case 2:{
			alumno.setCodigoPersonal(request.getParameter("CodigoPersonal"));
			if (!AlumUtil.existeReg(conEnoc, request.getParameter("CodigoPersonal")))break;
%>		
  <tr> 
    <td align="center">	  
	  <%out.print("1"); %>
