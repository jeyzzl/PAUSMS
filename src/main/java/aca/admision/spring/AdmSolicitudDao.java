	...
	public boolean insertReg(AdmSolicitud admSolicitud) throws Exception{
		...
		try{
			String comando = "INSERT INTO SALOMON.ADM_SOLICITUD"+ 
				...
				" RELIGION_ID, BAUTIZADO, USUARIO, CLAVE, FECHA, MATRICULA, EMAIL, ESTADO, ASESOR_ID, CURP, FECHA_INGRESO, AGENTE, TELEFONO,CODIGO, USUARIO_ID,"+
				" FECHA_BAUTIZO, LUGAR_BAUTIZO, CULTURAL_ID, REGION_ID, RES_PAIS_ID, RES_ESTADO_ID, RES_CIUDAD_ID, ACOMODO)"+
				" VALUES( TO_NUMBER(?,'9999999'), ?, ?, ?,TO_NUMBER(?,'999'), TO_NUMBER(?,'999'), TO_NUMBER(?,'999'), TO_NUMBER(?,'999')," +
				" TO_DATE(?,'DD/MM/YYYY'), ?, ?,TO_NUMBER(?,'99'), ?, ?, ?, sysdate, ?, ?, ?, ?, ?, ?, TO_NUMBER(?,'99'), ?, ?, ?,"+
				" ?, ?, TO_NUMBER(?,'999'), TO_NUMBER(?,'999'), TO_NUMBER(?,'999'), TO_NUMBER(?,'999'), TO_NUMBER(?,'999'), ?)";			
			Object[] parametros = new Object[] {
				admSolicitud.getFolio(),admSolicitud.getNombre(),admSolicitud.getApellidoPaterno(),admSolicitud.getApellidoMaterno(),admSolicitud.getPaisId(),admSolicitud.getEstadoId(),
				...
				admSolicitud.getBautizado(),admSolicitud.getUsuario(),admSolicitud.getClave(),admSolicitud.getMatricula(),admSolicitud.getEmail(),
				admSolicitud.getEstado(),admSolicitud.getAsesorId(),admSolicitud.getCurp(),admSolicitud.getFechaIngreso(),admSolicitud.getAgente(), 
				admSolicitud.getTelefono(),admSolicitud.getCodigo(), admSolicitud.getUsuarioId(), admSolicitud.getFechaBautizo(), admSolicitud.getLugarBautizo(), admSolicitud.getCulturalId(),
				admSolicitud.getRegionId(), admSolicitud.getResPaisId(), admSolicitud.getResEstadoId(), admSolicitud.getResCiudadId(), admSolicitud.getAcomodoId()
 		 	};
	...
	public boolean updateReg(AdmSolicitud admSolicitud) {
		...
		try{
	  		 ...
	  		 Object[] parametros = new Object[] {
				...
				admSolicitud.getEstado(),admSolicitud.getAsesorId(),admSolicitud.getCurp(),admSolicitud.getFechaIngreso(),admSolicitud.getAgente(),admSolicitud.getAsesorSec(),admSolicitud.getCodigo(),
				admSolicitud.getTelefono(), admSolicitud.getUsuarioId(), admSolicitud.getFechaBautizo(), admSolicitud.getLugarBautizo(), admSolicitud.getCulturalId(), admSolicitud.getRegionId(),
				admSolicitud.getResPaisId(), admSolicitud.getResEstadoId(), admSolicitud.getResCiudadId(), admSolicitud.getAcomodoId(), admSolicitud.getFolio()
		 	};
			if (enocJdbc.update(comando,parametros)==1){
				ok = true;
			}	   
		...
		try {
			...
			if (enocJdbc.queryForObject(comando,Integer.class, parametros)>=1){
				comando = " SELECT SOL.FOLIO, US.NOMBRE, US.APELLIDO_PATERNO, US.APELLIDO_MATERNO, US.CIUDAD_ID, US.ESTADO_ID,"
						...
						+ " SOL.FECHA_BAUTIZO, SOL.LUGAR_BAUTIZO, SOL.CULTURAL_ID, SOL.REGION_ID, SOL.RES_PAIS_ID, SOL.RES_ESTADO_ID, SOL.RES_CIUDAD_ID, SOL.ACOMODO_ID"
	...
	public List<AdmSolicitud> listIngreso( String orden ){
		...
		try{
			String comando =  "SELECT ASOL.FOLIO, ASOL.NOMBRE, ASOL.APELLIDO_PATERNO,"
				...
				+ " ASOL.EMAIL, ASOL.ASESOR_ID, ASOL.CURP, ASOL.FECHA_INGRESO, ASOL.AGENTE, ASOL.ASESOR_SEC, ASOL.TELEFONO, ASOL.CARTA,CODIGO,"
				+ " ASOL.USUARIO_ID,"
				+ " ASOL.FECHA_BAUTIZO, ASOL.LUGAR_BAUTIZO, ASOL.CULTURAL_ID, ASOL.REGION_ID, ASOL.RES_PAIS_ID, ASOL.RES_ESTADO_ID, ASOL.RES_CIUDAD_ID, ASOL.ACOMODO_ID"
