...
public class Requisitos {
    ...
    public boolean guardaRequisito(Connection conn, RequisitoVO requisito) throws SQLException{
    	...
    	boolean grabo=true;
    	try{
    	    comando="INSERT INTO ENOC.SSOC_REQUISITOS VALUES ((select COALESCE(max(REQUISITO_ID),0)+1 from ENOC.ssoc_requisitos),?,?)"; 
    	    ps = conn.prepareStatement(comando);
    	    ps.setString(1,requisito.getNombre());
    	    ps.setInt(2,requisito.getOrden());
    		ps.executeUpdate();
    	}catch(Exception ex){
    		System.out.println("Error - aca.ssoc.Requisitos|guardaRequisito|:"+ex);
    		grabo=false;
    	}
    	finally{
    ...
    public ArrayList<RequisitoVO> getRequisitos(Connection conn) throws SQLException{
    	...
    	ResultSet rs = null;
    	try{
    	    comando="select * from ENOC.SSOC_REQUISITOS order by orden"; 
    	    ps = conn.prepareStatement(comando);
    	    rs = ps.executeQuery();
    	    while(rs.next()){
    ...
    public RequisitoVO getRequisito(Connection conn,int id) throws SQLException{
    	...
    	ResultSet rs = null;
    	try{
    	    comando="select * from ENOC.SSOC_REQUISITOS "+ 
    	    		"where requisito_id=?";
    	    ps = conn.prepareStatement(comando);
    	    ps.setInt(1,id);
    	    rs = ps.executeQuery();
    		...
    		System.out.println("Error - aca.ssoc.Requisitos|getRequisito|:"+ex);
    	}
    	finally{
    	    try { ps.close(); } catch (Exception ignore) { }
    	    try { rs.close(); } catch (Exception ignore) { }
    ...
    public void modificaRequisito(Connection conn, RequisitoVO requisito) throws SQLException{
    	...
    	PreparedStatement ps=null;
    	try{
    	    comando="update ENOC.SSOC_REQUISITOS set " + 
    		...
    		System.out.println("Error - aca.ssoc.Requisitos|modificaRequisito|:"+ex);
    	}
    	finally{
    	    try { ps.close(); } catch (Exception ignore) { }
    	}
    ...
    public void eliminaRequisito(Connection conn, String id) throws SQLException{
    	...
    	PreparedStatement ps=null;
    	try{
    	    comando="delete from ENOC.SSOC_REQUISITOS " + 
    	    		"where requisito_id=?";
    	    ps = conn.prepareStatement(comando);
    	    ps.setString(1,id);
    	    ps.executeUpdate();
    		...
    		System.out.println("Error - aca.ssoc.Requisitos|eliminaRequisito|:"+ex);
    	}
    	finally{
    	    try { ps.close(); } catch (Exception ignore) { }
    	}
