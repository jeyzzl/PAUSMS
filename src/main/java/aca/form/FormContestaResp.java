...
public class FormContestaResp{
	...
	public boolean insertReg(Connection conn ) throws Exception{
		...
		try{
			ps = conn.prepareStatement("INSERT INTO "+
				"FORM_CONTESTA_RESP(CODIGO_PERSONAL, CARGA_ID, FORM_ID, PREGUNTA_ID, RESPUESTA_ID, ABIERTA) "+
				"VALUES( ?,?,?,?,?,?)");
			...
			System.out.println("Error - aca.form.FormContestaResp|insertReg|:"+ex);			
		}finally{
	...
	public boolean updateReg(Connection conn ) throws Exception{
		...
		PreparedStatement ps = null;
		try{
			ps = conn.prepareStatement("UPDATE FORM_CONTESTA_RESP "+
				"SET  ABIERTA = ? "+
			...
			System.out.println("Error - aca.form.FormContestaResp|updateReg|:"+ex);		
		}finally{
			try { ps.close(); } catch (Exception ignore) { }
		}
	...
	public boolean deleteReg(Connection conn ) throws Exception{
		...
		PreparedStatement ps = null;
		try{
			ps = conn.prepareStatement("DELETE FROM FORM_CONTESTA_RESP "+
				"WHERE CODIGO_PERSONAL = ? ");
			ps.setString(1, codigoPersonal);
			...
			System.out.println("Error - aca.form.FormContestaResp|deleteReg|:"+ex);			
		}finally{
			try { ps.close(); } catch (Exception ignore) { }
		}
		return ok;
	...
	public void mapeaRegId( Connection conn, String codigoPersonal, String respuestaId) throws SQLException{
		...
		try{
			ps = conn.prepareStatement("SELECT CODIGO_PERSONAL, CARGA_ID, FORM_ID, PREGUNTA_ID, ABIERTA "+
				"FROM FORM_CONTESTA_RESP WHERE CODIGO_PERSONAL = ?  RESPUESTA_ID = ? ");
			ps.setString(1,codigoPersonal);
			...
			System.out.println("Error - aca.form.FormContestaRep|mapeaRegId|:"+ex);
		}finally{
			if(rs!=null) rs.close();
			if(ps!=null) ps.close();
		}
	...
	public boolean existeReg(Connection conn) throws SQLException{
		...
		try{
			ps = conn.prepareStatement("SELECT * FROM FORM_CONTESTA_RESP WHERE CODIGO_PERSONAL = ? RESPUESTA_ID = ? ");
			ps.setString(1,codigoPersonal);
			ps.setString(2,respuestaId);
			rs = ps.executeQuery();
			if (rs.next())
			...
			System.out.println("Error - aca.form.FormContestaResp|existeReg|:"+ex);
		}finally{
	...
	public String maximoReg(Connection conn) throws SQLException{
		...
		try{
			ps = conn.prepareStatement("SELECT MAX(CODIGO_PERSONAL)+1 MAXIMO FROM FORM_CONTESTA_RESP"); 
		    ps = conn.prepareStatement("SELECT MAX(RESPUESTA_ID)+1 MAXIMO FROM FORM_CONTESTA_RESP" );			
			rs = ps.executeQuery();
			if (rs.next())
				maximo = rs.getString("MAXIMO");			
			
		}catch(Exception ex){
			System.out.println("Error - aca.form.FormContestaResp|maximoReg|:"+ex);
		}finally{
			try { rs.close(); } catch (Exception ignore) { }
			try { ps.close(); } catch (Exception ignore) { }
		}
