...
import aca.cert.spring.CertAlum;
import aca.cert.spring.CertAlumDao;
import aca.cert.spring.CertAlumNota;
import aca.cert.spring.CertAlumNotaDao;
...
@Controller
public class ContCertificacionCertificado {	
	...
	@Autowired
	private CertAlumDao certAlumDao;
	
	@Autowired
	private CertRelacionDao certRelacionDao;
	
	@Autowired
	private CertAlumNotaDao certAlumNotaDao;
	...
	@RequestMapping("/certificacion/certificado/alumno")
	public String certificacionCertificadoAlumno(HttpServletRequest request, Model modelo){	
		String codigoAlumno = "";
		...
		String curpAlumno		= alumPersonalDao.getCurp(codigoAlumno);
		String completo			= request.getParameter("Completo")==null?"C":request.getParameter("Completo");
		...
		double pglobal			= 0;
		double pglobalF     	= 0;
		float notaCreditos		= 0;
		boolean updateCerAlum  	= false;
		boolean insertaCerAlum 	= false;
		boolean esAlumno	 	= false;
		boolean existeCertPlan	= false;
		
		int matGrabadas=0, matSugeridas=0;
		
		AlumPlan alumPlan 			= new AlumPlan();
		CertAlum certAlum 			= new CertAlum();
		...
		List<String> lisPlan 		= new ArrayList<String>();

		if(codigoAlumno.charAt(0) != '9'){
			esAlumno = true;
			...
			if(certAlumDao.existeReg(codigoAlumno, planId)){
				certAlum = certAlumDao.mapeaRegId(codigoAlumno,planId);
				strFecha = certAlum.getFecha();
			}
			...
			if(certPlanDao.existeReg(planId)){
				...
				lisPlan = alumPlanDao.getPlanesAlumno(codigoAlumno);
				
	// ************************* Guardar las Notas *****************
				if(accion.equals("1")){
					List<CertCurso> lisCursos 	= certCursoDao.getListPlan(planId, "ORDER BY CICLO_ID, ORDEN");				
					String numCursos		= request.getParameter("numCursos");
					float creditos 			= 0;				
					boolean error			= false;
					
					for(int i = 0; i < lisCursos.size(); i++){
						certCurso = lisCursos.get(i);
						String nota 		= request.getParameter("nota-"+certCurso.getCursoId())==null?"":request.getParameter("nota-"+certCurso.getCursoId());
						String notaLetra 	= request.getParameter("letra-"+certCurso.getCursoId())==null?"":request.getParameter("letra-"+certCurso.getCursoId());
						
						if(!nota.equals("") && notaLetra.equals("")){
							notaLetra = "";
							if (nota.equals("10.0")){
								notaLetra = "diez punto cero";
							}else{
								for(int j = 0; j < nota.length(); j++){
									char n = nota.charAt(j);
									if(n == '9'){
										notaLetra += "nueve ";
									}else if(n == '8'){
										notaLetra += "ocho ";
						...
						String promedia 	= request.getParameter("mats-"+certCurso.getCursoId());
						if (certCurso.getTipoCursoId().equals("7") || certCurso.getTipoCursoId().equals("2") ){
							optativa = request.getParameter("optativa-"+certCurso.getCursoId())==null?"":request.getParameter("optativa-"+certCurso.getCursoId());				
						}else{
							optativa = "";
						}
						
						if(nota != null && !nota.equals("0.0")){
							if(!nota.trim().equals("")){	
								...
								if(certAlumNotaDao.existeReg( codigoAlumno, certCurso.getCursoId())){								
									certAlumNota = certAlumNotaDao.mapeaRegId(codigoAlumno, certCurso.getCursoId());
									certAlumNota.setCurso(request.getParameter("curso-"+certCurso.getCursoId())==null?"":request.getParameter("curso-"+certCurso.getCursoId()));
									certAlumNota.setPromedia(request.getParameter("mats-"+certCurso.getCursoId())==null?"N":"S");								
									certAlumNota.setNota(nota);
									certAlumNota.setNotaLetra(notaLetra);
									certAlumNota.setOptativa(optativa);
									if(!certAlumNota.getCurso().equals("")) {
										if(!certAlumNotaDao.updateReg( certAlumNota)) {
										error = true;
										}
									}
								}else{								
									certAlumNota.setFolio(certAlumNotaDao.maxReg(codigoAlumno));
									certAlumNota.setPlanId(planId);
									certAlumNota.setCicloId(certCurso.getCicloId());
									certAlumNota.setCurso(request.getParameter("curso-"+certCurso.getCursoId())==null?"":request.getParameter("curso-"+certCurso.getCursoId()));
									certAlumNota.setEstado("A");
									certAlumNota.setNota(nota);
									certAlumNota.setNotaLetra(notaLetra);
									certAlumNota.setOptativa(optativa);
									certAlumNota.setPromedia(request.getParameter("mats-"+certCurso.getCursoId())==null?"N":"S");		
									if(!certAlumNota.getCurso().equals("")) {
										if(!certAlumNotaDao.insertReg( certAlumNota)){
											error = true;									
										}									
									}									
								}
					...
					if(!error){			
						certAlum.setCodigoPersonal(codigoAlumno);
						certAlum.setPlanId(planId);					
						if(certAlumDao.existeReg(codigoAlumno, planId)){						
							certAlum = certAlumDao.mapeaRegId(codigoAlumno, planId);						
							certAlum.setAvance(completo);
							certAlum.setNumCursos(numCursos);
							certAlum.setEquivalencia(request.getParameter("Equivalencia").equals("")?"NA":request.getParameter("Equivalencia"));												
							certAlum.setEstado("A");
							certAlum.setFecha(request.getParameter("Fecha"));
							certAlum.setEncabezado(request.getParameter("Encabezado"));
							certAlum.setLinea(lineas);
							
							if(certAlumDao.updateReg(certAlum)){
								updateCerAlum = true;
							}else{
								error = true;
							}
							...
							certAlum.setAvance(completo);
							certAlum.setNumCursos(String.valueOf(numCursos));
							certAlum.setFecha(request.getParameter("Fecha"));
							certAlum.setEquivalencia(request.getParameter("Equivalencia").equals("")?"NA":request.getParameter("Equivalencia"));
							certAlum.setEstado("A");
							certAlum.setEncabezado(request.getParameter("Encabezado"));
							if(certAlumDao.insertReg( certAlum)){
								insertaCerAlum = true;
							}else{
								error = true;
							}
...
//		************************Imprimir Pdf ************************
	/* C A N C E L A D O --- 
					String pdfMaterias 	= request.getParameter("pdfmaterias")==null?"-":request.getParameter("pdfmaterias");
					String promedio  	= request.getParameter("pdfpromedio")==null?"0":request.getParameter("pdfpromedio");
					String foto 		= request.getParameter("foto")==null?"Sin":request.getParameter("foto");
					session.setAttribute("Accion", "0");						
					response.sendRedirect("certificadoPDF?plan="+planId+"&pdfmaterias="+pdfMaterias+"&pdfpromedio="+promedio+"&foto="+foto+"&lineas="+request.getParameter("lineas"));	
	*/				
				}else if(accion.equals("3")){	
//		************************Cerra las notas grabadas*******************
					int numNotas = certAlumNotaDao.getNumAlumNotas(codigoAlumno,planId);
					if (numNotas > 0){
						certAlumNotaDao.cerrarNotas(codigoAlumno, planId);
					}				
				}else if(accion.equals("4")){	
//		************************Abrir las notas grabadas***********************
					int numNotas = certAlumNotaDao.getNumAlumNotas(codigoAlumno,planId);				
					if (numNotas > 0){					
						certAlumNotaDao.abrirNotas(codigoAlumno, planId);
					}				
				}else if(accion.equals("5")){ 
	//  ******************** Borrar notas ********************************				
					certAlumNota.setCodigoPersonal(codigoAlumno);
					certAlumNota.setPlanId(planId);
					if (certAlumNotaDao.deleteNotas( codigoAlumno, planId) ){
						certAlum = certAlumDao.mapeaRegId(codigoAlumno, planId);
						certAlum.setAvance("A");
						certAlum.setNumCursos("0");
						certAlum.setFecha(aca.util.Fecha.getHoy());
						if (certAlumDao.updateReg( certAlum)){
						}	
					}else{
						System.out.println("No borro las notas");
					}
				}else if(accion.equals("6")){
				//  ******************** Borrar renglon ********************************				
					certAlumNota.setCodigoPersonal(codigoAlumno);
					certAlumNota.setFolio(request.getParameter("folio"));
					if (certAlumNotaDao.deleteReg( codigoAlumno, request.getParameter("folio")) ){	
					}else{
						System.out.println("No borro la nota");
					}
				}
			}else {
				lisPlan = alumPlanDao.getPlanesAlumno(codigoAlumno);
		...
		int numAlumNotas = certAlumNotaDao.getNumAlumNotas(codigoAlumno, planId);
		
		HashMap<String, CertAlumNota> mapCertAlumNotas 	= certAlumNotaDao.mapCertAlumNotas(codigoAlumno);
		HashMap<String,CertRelacion> mapCertRelacion 	= certRelacionDao.mapCertRelacion(lisCursos);
		HashMap<String,AlumnoCurso> mapTieneCurso 		= alumnoCursoDao.mapTieneCurso(codigoAlumno);
		HashMap<String, Carga> mapaCargas 				= cargaDao.mapaCargas();
		HashMap<String,AlumPlan> mapAlumIngreso 		= alumPlanDao.mapAlumIngreso(codigoAlumno);
		HashMap<String,String> mapaNombrePlan	 		= new HashMap<String,String>();
		
		for(String plan : lisPlan) {
			mapaNombrePlan.put(plan, mapaPlanDao.getNombrePlan(plan));
		}
		
		modelo.addAttribute("esAlumno", esAlumno);
		modelo.addAttribute("existeCertPlan", existeCertPlan);
		modelo.addAttribute("updateCerAlum", updateCerAlum);
		modelo.addAttribute("insertaCerAlum", insertaCerAlum);
		modelo.addAttribute("certAlum", certAlum);
		modelo.addAttribute("strFecha", strFecha);
		modelo.addAttribute("strCheck", strCheck);
		...
		modelo.addAttribute("fechaNota", fechaNota);
		modelo.addAttribute("optativa", optativa);
		modelo.addAttribute("cursoLlevado", cursoLlevado);
		modelo.addAttribute("matGrabadas", matGrabadas);
		modelo.addAttribute("matSugeridas", matSugeridas);
		modelo.addAttribute("totCreditos", totCreditos);
		modelo.addAttribute("pglobal", pglobal);
		modelo.addAttribute("credProm", credProm);
		modelo.addAttribute("notaCreditos", notaCreditos);
		modelo.addAttribute("pglobalF", pglobalF);
		modelo.addAttribute("mapCertAlumNotas", mapCertAlumNotas);
		modelo.addAttribute("mapCertRelacion", mapCertRelacion);
		modelo.addAttribute("mapTieneCurso", mapTieneCurso);
		modelo.addAttribute("mapaCargas", mapaCargas);
		modelo.addAttribute("mapAlumIngreso", mapAlumIngreso);
		modelo.addAttribute("numAlumNotas", numAlumNotas);
		modelo.addAttribute("mapaNombrePlan", mapaNombrePlan);
				
		return "certificacion/certificado/alumno";
	}
	
	@RequestMapping("/certificacion/certificado/certificadoPDF")
	public void certificacionCertificadoCertificadoPDF(HttpServletRequest request, HttpServletResponse response) throws DocumentException, IOException{
		String codigoAlumno = "";
		...
		String carpeta 	= context.getRealPath("/WEB-INF/pdf/certificado/");
		if(!new java.io.File(carpeta).exists()) new java.io.File(carpeta).mkdirs();
		String dir 			= carpeta+codigoAlumno+planId.replace("*","")+".pdf";
		
		String curpAlumno	= alumPersonalDao.getCurp(codigoAlumno);
		
		String carreraId	= mapaPlanDao.getCarreraId(planId);
		String numNivel     = catCarreraDao.getNivelId(carreraId);
...
//			------------------------------ Termina fijar tamaños y posiciones de las columnas -------------------------	
			...
			for(int i = 0; i < lisCiclos.size(); i++){
				...
				if (certAlumNotaDao.getNumMatCiclo(codigoAlumno, planId, certCiclo.getCicloId()) > 0){
		...
		// ---->>>>>>>>>>>>>>>>>>>>>>>Tabla "Requisitos Complementarios"
		    	...
		    	if (numComp > 0){
					...
					int[] comp = alumnoCursoDao.getArrayComponente(codigoAlumno, planId, "SC");
					String texto = "";
					...
					comp = alumnoCursoDao.getArrayComponente(codigoAlumno, planId, "AF");
					texto = "";
					...
					comp = alumnoCursoDao.getArrayComponente(codigoAlumno, planId, "TM");
					texto = "";
					...
					comp = alumnoCursoDao.getArrayComponente(codigoAlumno, planId, "LC");
					texto = "";
...
//				------------------------------ Construye el Parrafo Final -------------------------
		    	...
		    	frase = new Phrase();
				List<String> frases = certPlanDao.getFrases(certPlan.gettFinal());
