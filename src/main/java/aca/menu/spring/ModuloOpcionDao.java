...
public class ModuloOpcionDao {
	...
	public boolean updateOpcion(ModuloOpcion moduloOpcion){
		boolean ok = false;        
		try{
			String comando = "UPDATE ENOC.MODULO_OPCION SET NOMBRE_INGLES = ?, NOMBRE_OPCION = ?, ICONO = ?, USUARIOS = ? WHERE OPCION_ID = ?";            
			Object[] parametros = new Object[] {moduloOpcion.getNombreIngles(), moduloOpcion.getNombreOpcion(), moduloOpcion.getIcono(), moduloOpcion.getUsuarios(), moduloOpcion.getOpcionId()};	
			if (enocJdbc.update(comando,parametros)==1){
				ok = true;			
			}     
		}catch (Exception ex){
	...
	public List<ModuloOpcion> lisPorEstado( String estados, String orden){		
		List<ModuloOpcion> lista 	= new ArrayList<ModuloOpcion>();
		try{
			String comando = " SELECT MODULO_ID, OPCION_ID, NOMBRE_OPCION, URL, COALESCE(ICONO,' ') AS ICONO, ORDEN, USUARIOS, CARPETA, NOMBRE_INGLES"
					+ " FROM ENOC.MODULO_OPCION WHERE ICONO IN ("+estados+") "+orden;					
			lista = enocJdbc.query(comando, new ModuloOpcionMapper());			
		}catch(Exception ex){
			System.out.println("Error - aca.menu.spring.ModuloOpcionDao|lisTodas|:"+ex);
		}
