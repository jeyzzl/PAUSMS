...
public class AccesoOpcionDao {
	...
	public int maestrosSinPortal(String opcionId){	
		...
		try{
			...
			Object[] parametros = new Object[] {opcionId};
			totPrincipal 	= enocJdbc.queryForObject(comando, Integer.class, parametros);						
			comando = "SELECT COUNT(DISTINCT(CODIGO_OTRO)) FROM ENOC.CARGA_GRUPO WHERE CODIGO_OTRO NOT IN (SELECT CODIGO_PERSONAL FROM ENOC.ACCESO_OPCION WHERE OPCION_ID = ?)";
			totAuxiliar 	= enocJdbc.queryForObject(comando, Integer.class, parametros);						
		}catch(Exception ex){
			System.out.println("Error - aca.acceso.spring.AccesoOpcionDao|maestrosSinPortal|:"+ex);
		}		
		return totPrincipal+totAuxiliar;
	}
	...
	public List<String> lisAuxiliaresSinPortal( String opcionId ){
		List<String> lista = new ArrayList<String>();
		try{
			String comando = "SELECT DISTINCT(CODIGO_OTRO) FROM ENOC.CARGA_GRUPO WHERE CODIGO_OTRO NOT IN (SELECT CODIGO_PERSONAL FROM ENOC.ACCESO_OPCION WHERE OPCION_ID = ?)";
			lista = enocJdbc.queryForList(comando, String.class, opcionId);
		}catch(Exception ex){
			System.out.println("Error - aca.acceso.spring.AccesoOpcionDao|lisAuxiliaresSinPortal|:"+ex);
		}
		return lista;
	}
	...
	public HashMap<String, String> mapOpciones(){
		HashMap<String, String> mapa = new HashMap<String, String>();
		List<aca.Mapa> lista			= new ArrayList<aca.Mapa>();		
		try{
			String comando = "SELECT CODIGO_PERSONAL AS LLAVE, COUNT(OPCION_ID) AS VALOR  FROM ENOC.ACCESO_OPCION GROUP BY CODIGO_PERSONAL";
			lista = enocJdbc.query(comando, new aca.MapaMapper());
			for (aca.Mapa map : lista){
				mapa.put(map.getLlave(), map.getValor());
			}
		}catch(Exception ex){
			System.out.println("Error - aca.acceso.spring.AccesoOpcionDao|mapOpciones|:"+ex);
		}
