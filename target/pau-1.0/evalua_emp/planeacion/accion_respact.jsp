...
<div class="container-fluid">
	...
	<form action="" method="post" name="accion_meta">
        ...
        <input name="TKL_id_actividad" type="hidden" id="TKL_id_actividad" value="<%= request.getParameter("TKL_id_actividad")%>">
        ...
        <p>
        	<label for="clave">Seleccione un empleado</label>
        	<select name="clave" id="clave" class="input-xxlarge">
	        	<optgroup label="Personal depto">
	            <%
	          		Iterator<DptoPersonal> itPersonal = opres.listaEmpleadosDpto((String)session.getAttribute("TKL_ccosto"), (String)session.getAttribute("TKL_ejercicio")).iterator();
	          		while(itPersonal.hasNext()){
	              		DptoPersonal personal= itPersonal.next();
	            %>
	                	<option value="<%= personal.getClave() %>"><%= personal.getAppaterno() %> <%= personal.getApmaterno() %>, <%= personal.getNombre() %></option>  
	            <%
	          		}
	            %>
	                </optgroup>
	            ...
	            <% 
	          		Iterator<DptoPersonal> itPersonalG = opres.listaEmpleadosGeneral( (String)session.getAttribute("TKL_ccosto"), (String)session.getAttribute("TKL_ejercicio")).iterator();
