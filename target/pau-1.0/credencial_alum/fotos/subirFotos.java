...
public class subirFotos extends Applet 
{
  private FileOutputStream fileOutStream;		
	...
	{    
			...
			int cont=0;	
			try{
				 cont = Subir("c:\\Credenciales\\Fotos");
  ...
  { 
			...
			try{
				...
				System.out.println("OK!");					
				tFotos=Lista.length;
				flog = new FileOutputStream("C:\\Credenciales\\Subir.txt");
				for (int i=0;i<Lista.length || !bCancel;i++)
				{ 
						if (i%100==0){
			  	  ...
			  	  COMANDO = "select foto from ENOC.alum_personal Where codigo_personal = '"+matricula+"'"; 
				    rset = stmt.executeQuery(COMANDO);		
						nFotos=i;
						lblMensaje.setText(ListaArchivos[i].getName());
						lblProgreso.setText((int)((double)nFotos/(double)tFotos*100)+1 + "% Completado...");						
			    	if (rset.next()){
						  	...
						  	try{
									COMANDO = "update ENOC.alum_personal set foto = empty_blob() Where codigo_personal = '"+matricula+"'"; 
								  rset = stmt.executeQuery(COMANDO);
							  	COMANDO = "select foto from ENOC.alum_personal Where codigo_personal = '"+matricula+"' for update"; 
								  rset = stmt.executeQuery(COMANDO);
							    rset.next();					
							    BLOB blob=((OracleResultSet)rset).getBLOB("foto");
