...
%>		
		<tr>			
			<td>
<%			if (autorizar){ %>			
				<a onclick="javascript:autorizar('<%= proyectos.getProyectoId() %>');" class="btn btn-success btn-sm" title="Autorizar"><i class="fas fa-check"></i></a>&nbsp;
<%			} %>				
<%			if (cancelar){ %>
				<a onclick="javascript:cancelar('<%= proyectos.getProyectoId() %>');" class="btn btn-danger btn-sm" title="Cancelar"><i class="fas fa-arrow-left"></i></a>&nbsp;
<%			}

			if(mapaComentarios.containsKey(proyectos.getProyectoId())){
				if(Integer.parseInt(mapaComentarios.get(proyectos.getProyectoId()))>= 1){
%>
				<a href="comentario?ProyectoId=<%= proyectos.getProyectoId() %>" class="btn btn-primary btn-sm" title="Comentario"><i class="fas fa-comment-alt"></i></a>&nbsp;
<% 				}
			}else{ %>
				<a href="comentario?ProyectoId=<%= proyectos.getProyectoId() %>" class="btn btn-info btn-sm" title="Comentario"><i class="fas fa-comment-alt"></i></a>&nbsp;
<% 			}%>			
				<a href="bitacora?ProyectoId=<%= proyectos.getProyectoId() %>" class="btn btn-warning btn-sm" title="Bitacora"><i class="fas fa-book"></i></a>
<%
			if (nombreEstado != "5"){
%>
				<a onclick="javascript:concluir('<%= proyectos.getProyectoId() %>')" class="btn btn-success btn-sm" title="Concluir"><i class="fas fa-arrow-up"></i></a>&nbsp;				
<%
			}
...
<script>	
	...
	function concluir(proyectoId){
		if(confirm("¡Esta acción concluye el proyecto! ¿Deseas continuar?")){
			location.href = "accion?Accion=3&ProyectoId="+proyectoId;
		}
	}
</script>