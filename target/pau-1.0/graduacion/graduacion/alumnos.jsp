...
<script type="text/javascript">
	function borrar(codigoPersonal, eventoId){
		if (confirm("¿Esta seguro que desea borrar al alumno?")){
...
<div class="container-fluid">
	<h2>Alumnos graduados por evento</h2>
	<form name="frmGraduacion">
	<div class="alert alert-info d-flex align-items-center">
		<b>Evento:</b>
		<select class="form-select" id="EventoId" name="EventoId" onchange="javascript:recarga()" style="width:350px">
...
<%	}%>
		...
		&nbsp;&nbsp;
		<b>Alumno:</b>
		<input class="form-control" type="text" id="CodigoPersonal" name="CodigoPersonal" value="<%=codigoPersonal%>" onkeyup="if(window.event.keyCode == 13) Nuevo(); return false;" maxlength="7" size="8" style="width:120px" /> 
		&nbsp;<a class="btn btn-primary" href="javascript:Nuevo()"><i class="icon-white icon-plus"></i> Agregar</a>&nbsp;&nbsp;
		&nbsp;&nbsp;<a class="btn btn-primary" href="updateProm?EventoId=<%=eventoId%>"><i class="fas fa-sync-alt"></i> Promedio</a>
		&nbsp;&nbsp;<a class="btn btn-primary" href="verificar?EventoId=<%=eventoId%>"><i class="fas fa-sync-alt"></i> Avance</a>
		&nbsp;&nbsp;
		...
		&nbsp;&nbsp;
		<input type="text" class="form-control" placeholder="Buscar..." id="buscar" style="width:140px">		
	</div>	
	</form>
<%	if(!mensaje.equals("-")){ %>
...
%>
	<thead>
		...
		<tr class="table-dark">
			<th style="text-align: center"><spring:message code="aca.Operacion"/></th>
			<th style="text-align: center">Permiso</th>
			<th width="2%" style="text-align:center">Confirmar</th>
...
<% 			}%>
			...
			<th width="5%" style="text-align: center">División</th>							
...
%>
		...
		<tr align="center">
			...
			<th>Prom.</th>
			<th>Tit.</th>
			<th width="5%" align="center">División</th>
			<th><spring:message code="aca.Estado"/></th>
			<th><spring:message code="aca.Pais"/></th>
			<th>¿Inscrito?</th>
			<th>Tipo Alumno</th>
...
<%			
		...
		if (alumno.getPermiso().equals("S")) colorPermiso = "<span class='badge bg-success rounded-pill'>SI</span>";
...
<%		if (codigoUsuario.equals("9800308") || codigoUsuario.equals("9800385")){ %>	
				<a href="cambiar?EventoId=<%=alumno.getEventoId()%>&CodigoAlumno=<%=alumno.getCodigoPersonal()%>"><span class="badge <%=alumno.getConfirmar().equals("N")?"bg-danger":"bg-success"%> rounded-pill"><%=alumno.getConfirmar().equals("N")?"NO":"SI"%></span></a>
<%		}else{ %>
				<span class="badge <%=alumno.getConfirmar().equals("N")?"bg-danger":"bg-success"%> rounded-pill"><%=alumno.getConfirmar().equals("N")?"NO":"SI"%></span>		
<%		}%>
			</td>
