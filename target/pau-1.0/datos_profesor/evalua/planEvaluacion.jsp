...
<jsp:useBean id="CargaUnidadU" class="aca.carga.CargaUnidadUtil" scope="page"/>
<% 
	...
	if (numUnidades==0){
		...
		for (int i=1; i<=numNewUnidades;i++){
			String strUnidad = "";
			
			/*Mapear la unidad en los planes nuevos*/
			aca.plan.MapaNuevoUnidad newUnidad  = new aca.plan.MapaNuevoUnidad();
			aca.plan.MapaNuevoUnidadUtil newUnidadU  = new aca.plan.MapaNuevoUnidadUtil();
			
			newUnidad.setCursoId(newCursoId);
			newUnidad.setUnidadId(String.valueOf(i));
			if (newUnidadU.existeReg(conEnoc, newCursoId, String.valueOf(i))){
				
				newUnidad.mapeaRegId(conEnoc, newCursoId, String.valueOf(i));
				...
				strUnidad = String.valueOf(i).length()==1?"0"+String.valueOf(i):String.valueOf(i);
				
				cargaUnidad.setCursoCargaId(cursoCargaId);
				cargaUnidad.setUnidadId(strUnidad);
				cargaUnidad.setOrden(String.valueOf(i));
	...
	ArrayList lisUnidad       	= UnidadU.getListUnidad(conEnoc, cursoCargaId, "ORDER BY ORDEN");
	ArrayList lisCompetencias  	= UnidadCompU.getListCompetencias(conEnoc, cursoCargaId, "ORDER BY COMPETENCIA_ID");
	ArrayList lisTemas         	= TemaU.getListTema(conEnoc, cursoCargaId, "ORDER BY ORDEN");
    ArrayList lisActividades   	= ActividadU.getListActividades(conEnoc, cursoCargaId, "ORDER BY ACTIVIDAD_ID");
	ArrayList lisCriterios     	= CriterioU.getListCriterio(conEnoc, cursoCargaId, "ORDER BY CRITERIO_ID");
	ArrayList lisInstrumentos  	= InstrumentoU.getListInstrumento(conEnoc, cursoCargaId, "ORDER BY INSTRUMENTO_ID");
...
%> 
<table style="margin: 0 auto;  width:75%" style=" border-bottom: dotted 3px green;">
  <tr><td>&nbsp;</td></tr>
  <tr>
    <td style="background-color: #5FB404;font-size: 10pt; color:white;" align="center" colspan="5"  width="50%">
      ...
      <% }%>
      <b><%= unidad.getUnidadNombre().toUpperCase()%></b></td>
  </tr>
<%
		if (lisUnidad.size() > 0){
  ...
  <tr>
	<td colspan="5" align="center">
		<table style="width:75" align="center">
			<tr>
				<td class="header2" colspan="2">Competencias</td>
			</tr>
<%
			String tipo = "";
...
%>
  ...
  <tr>
	 <td width="10%" style="background-color: #EDED97;font-weight: bold;" align="center"><strong><spring:message code="aca.Fecha"/></strong></td>
	 <td width="20%" style="background-color: #EDED97;font-weight: bold;" align="center"><strong>Tema</strong></td>
	 <td width="25%" style="background-color: #EDED97;font-weight: bold;" align="center"><strong>Experiencias de Aprendizaje</strong></td>
	 <td width="25%" style="background-color: #EDED97;font-weight: bold;" align="center"><strong></strong></td>
	 <td width="5%" style="background-color:  #EDED97;font-weight: bold;" align="center"><strong>Valor</strong></td>
  </tr>
