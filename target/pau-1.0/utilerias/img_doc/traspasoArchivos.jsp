...
<% 
	...
	COMANDO = "SELECT ARCHIVO_ID, FOLIO, CODIGO_PERSONAL, FECHA, NOMBRE, COMENTARIO, TAMANO, STATUS, ARCHIVO "+
			"FROM PORTAL.ARCHIVOS_ALUMNO "+ 
			"WHERE ARCHIVO IS NOT NULL "+
			//"AND ARCHIVO_ID > '05062B-0049-5' "+
			"ORDER BY 1";
//			
...
*/		
		...
		// Pasar el obj a UNAV
		COMANDO = "select archivo_id from PORTAL.archivos_alumno where archivo_id = '"+archivoId+"' and folio = "+folio+" and codigo_personal = '"+matricula+"' "; 
		rsetU = stmtUnav.executeQuery(COMANDO);
		if (rsetU.next()){
  			COMANDO = "update PORTAL.archivos_alumno set archivo = ? where archivo_id = '"+archivoId+"' and folio = "+folio+" and codigo_personal = '"+matricula+"' "; 
  			operacion = 1;
		}else{
			operacion = 2;
			COMANDO = "insert into PORTAL.ARCHIVOS_ALUMNO(archivo_id,folio,codigo_personal,fecha,nombre,comentario,tamano,status,archivo)"+ 
					  "Values ('"+archivoId+"',"+folio+",'"+matricula+"','"+fecha+"','"+nombre+"','"+comentario+"',"+tamano+",'"+status+"',?)";
		}
