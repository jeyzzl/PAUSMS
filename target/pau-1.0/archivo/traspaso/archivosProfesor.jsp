...
<script>
	function Traspasar(){
		if (confirm("¿ Deseas traspasar todos los datos ?")){
			document.location.href= "archivosProfesor?Accion=1";
		}
	}
	function subir(){
		if (confirm("¿ Deseas subir todos los datos ?")){
			document.location.href= "archivosProfesor?Accion=2";
		}
	}
</script>
<%
	...
	try{		
		Class.forName("org.postgresql.Driver");
		conn3 = DriverManager.getConnection("jdbc:postgresql://172.16.251.11:5432/archivo","postgres","jete17");
	...
	if (!error && accion.equals("1")){		
		COMANDO = "SELECT COUNT(*) AS TOTAL FROM PORTAL.ARCHIVOS_PROFESOR";
		rset2 = stmt2.executeQuery(COMANDO);
		if (rset2.next()){			 	
			total = rset2.getInt(1);
		}
		
		COMANDO = "SELECT ARCHIVO_ID,FOLIO,CODIGO_PERSONAL,TO_CHAR(FECHA,'DD/MM/YYYY') AS FECHA,NOMBRE,COMENTARIO,TAMANO,STATUS FROM PORTAL.ARCHIVOS_PROFESOR";
		rset2 = stmt2.executeQuery(COMANDO);
		while (rset2.next()){
			...
			String autorizacion		= rset2.getString("AUTORIZACION");
			String tamano			= rset2.getString("TAMANO");
			
			String comando = "INSERT INTO PORTAL.ARCHIVOS_PROFESOR(ARCHIVO_ID,FOLIO,CODIGO_PERSONAL,FECHA,NOMBRE,COMENTARIO,AUTORIZACION,TAMANO)"
				+" VALUES('"+archivoId+"',"+folio+",'"+codigoPersonal+"',TO_DATE('"+fecha+"','DD/MM/YYYY'),'"+nombre+"','"+comentario+"','"+autorizacion+"','"+tamano+"')";
	...
	if (!error2 && accion.equals("2")){
		...
		String path = application.getRealPath("archivo/traspaso/resArchProf");
	    ...
	    for (File fileEntry : folder.listFiles()) {
	        if (fileEntry.isDirectory()) {
				...
				*/
				...
				PreparedStatement ps = conn3.prepareStatement("UPDATE PORTAL.ARCHIVOS_PROFESOR SET ARCHIVO = ?"+
		...
		mensaje = "Se traspasaron "+numRow+" renglones a la tabla de PORTAL.ARCHIVOS_PROFESOR";
	}
	
%>
<div class="container-fluid">
	<h1>Archivos de maestros(Datos/archivos)</h1>
