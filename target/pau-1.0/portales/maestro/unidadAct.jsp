		    	...
		    	if (actividad!= null){
			    	...
			    	if (UnidadActU.existeReg(conEnoc, cursoCargaId, temaId+act.getActividadId() ) == false){
						if (UnidadActU.insertReg(conEnoc, UnidadAct)){
							Resultado = "Saved: "+UnidadAct.getCursoCargaId();
						}else{
							Resultado = "Error saving: "+UnidadAct.getCursoCargaId();
						}
					}else{	
						if (UnidadActU.updateReg(conEnoc, UnidadAct)){ 
							Resultado = "Strategy:"+UnidadAct.getCursoCargaId()+" updated";
						}else{
		...
		case 3:{//borrar			
			...
			if (UnidadActU.existeReg(conEnoc, cursoCargaId, actividadId) == true){
				if (UnidadActU.deleteReg(conEnoc, cursoCargaId, actividadId)){
					Resultado = "Deleted: "+UnidadAct.getCursoCargaId();
				}else{
					Resultado = "Error deleting: "+UnidadAct.getCursoCargaId();
				}	
			}else{
				Resultado = "Not found: "+UnidadAct.getCursoCargaId();
			} 
			break;
		}	   
	}
...
<form name="frmActividad" method="post" action="unidadAct?CursoCargaId=<%=cursoCargaId%>&Unidad=<%= unidadId %>&Tema=<%=temaId%>">
  ...
  <table style="margin: 0 auto;  width:70%"> 
    <tr><td style="font-size:12pt;" align="center" colspan="3">Subject: <%=materia%></td></tr>  
...
%>    	<tr>
		  ...
		  <%}else{ %>
		    ...
		    <input type="text" id="Descripcion" name="Descripcion" value="<%= unidAct.getActividadNombre() %>" />(Other)
		  <%} %> 
		  </td>
		  <td align="center"> 
