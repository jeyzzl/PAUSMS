...
<% 
	...
	ArrayList<aca.kardex.KrdxCursoCal> lisDiferida	= CalU.getListDiferidas(conEnoc, cursoCargaId,"D", "ORDER BY CODIGO_PERSONAL");
	...
	String rutaCarpeta 	= application.getRealPath("/WEB-INF/pdf/diferida/");
	...
	try{
        ...
        document.addSubject("Delayed Grading "+materia);
		document.open();
	    ...
	    Phrase contrato = new Phrase( "Delayed Grading Agreement ", FontFactory.getFont(FontFactory.HELVETICA, 14, Font.BOLD, new BaseColor(0,0,0)) );   	
        ...
        parrafo.add(new Phrase(fecha.getDia(aca.util.Fecha.getHoy())+" días"
        		, FontFactory.getFont(FontFactory.HELVETICA, 10, Font.BOLD, new BaseColor(r,g,b))));
        parrafo.add(new Phrase(" in the month of "
        		, FontFactory.getFont(FontFactory.HELVETICA, 10, Font.NORMAL, new BaseColor(r,g,b))));
        ...
        parrafo.add(new Phrase(yearName
        		, FontFactory.getFont(FontFactory.HELVETICA, 10, Font.BOLD, new BaseColor(r,g,b))));
        parrafo.add(new Phrase(" the present document has been issued requesting to delay"+
        		" the delivery of grades of the following subject to the date mentioned above.\nIf the student does not comply with his/her"+
        		" agreement, he/she will receive the grade recorded here. Once the requirements of the subject have been fulfilled, the " +
        		" coordination will prepare an ordinary grade report in which the corresponding grade will be recorded."
        		, FontFactory.getFont(FontFactory.HELVETICA, 10, Font.NORMAL, new BaseColor(r,g,b))));
		...
		carrera.add(new Phrase("and corresponding to the semester: "
        		, FontFactory.getFont(FontFactory.HELVETICA, 10, Font.NORMAL, new BaseColor(r,g,b))));
		...
		celda = new PdfPCell(new Phrase("Sub.Type"
				, FontFactory.getFont(FontFactory.HELVETICA, 8, Font.BOLD, new BaseColor(r,g,b))));
		celda.setHorizontalAlignment(Element.ALIGN_CENTER);
		celda.setBorder(Rectangle.BOX);
		celda.setBackgroundColor(new BaseColor(230,230,230));
		...
		celda = new PdfPCell(new Phrase("Student signature"
				, FontFactory.getFont(FontFactory.HELVETICA, 8, Font.BOLD, new BaseColor(r,g,b))));
		celda.setHorizontalAlignment(Element.ALIGN_CENTER);
		celda.setBorder(Rectangle.BOX);
		celda.setBackgroundColor(new BaseColor(230,230,230));
		...
		for(int i=0;i<lisDiferida.size();i++){
			...
			celda = new PdfPCell(new Phrase(aca.alumno.AlumUtil.getNombreAlumno(conEnoc,cal.getCodigoPersonal(),"NAME")
					, FontFactory.getFont(FontFactory.HELVETICA, 8, Font.NORMAL, new BaseColor(r,g,b))));
			celda.setHorizontalAlignment(Element.ALIGN_LEFT);
			celda.setBorder(Rectangle.BOX);
			celda.setColspan(1);
		...
		celda = new PdfPCell(new Phrase("Professor's Signature"
				, FontFactory.getFont(FontFactory.HELVETICA, 8, Font.BOLD, new BaseColor(r,g,b))));
		celda.setHorizontalAlignment(Element.ALIGN_CENTER);
		celda.setBorder(Rectangle.NO_BORDER);
		celda.setColspan(1);
		...
		System.err.println("Error while generating the PDF delayed grades format: "+ioe.getMessage());
	}
