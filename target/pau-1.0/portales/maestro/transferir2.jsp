...
<script>
	...
	function borra(parametros){
		...
		var checados=0;
		if(document.forma.listaEliminar!=undefined){
		  if (confirm("xEsta seguro que desea borrar estos archivos?")){
...
<%	
	...
	if(accion.equals("1")&&vista.equals("xEstrategia")){
		...
		MultipartRequest multi=null;
	        ...
	        multi = new MultipartRequest(request,ruta,7*1024*1024); 
...
%>
						<a href='<%=pRegreso%>'><b>&nbsp;&nbsp;<spring:message code="aca.Regresar"/></b></a>
				<br>
  <table style="width:97%" cellpadding="0" cellspacing="0" class="table table-fullcondensed table-nohover">
	<tr valign="top">
		<th colspan="10">
...
%>
				...
				<%if(error==1)out.print("<div align='center'><font color='red'>El tamaxo mxximo del archivo debe de ser de 7 Mb.</font></div>");%>
				</td>
			</tr>
...
<%			}if(!vista.equals("paraAlumno")&&!vista.equals("paraAlumnoxEstrategia")){%>
			<tr>
				<td colspan='5'><img class="button" onClick="borra('&vista=<%=vista%>&matricula=<%=matricula%>&id=<%=id%>&origen=Profesor&sCursoCargaId=<%=sCursoCargaId%>&al=<%=para%>&alumno=<%=alumno%>&nomProfesor=<%=nomProfesor%>&nomMateria=<%=nomMateria%>');" src="/academico/imagenes/borrararch.gif" border='0'/></td>
			</tr>
			<%}%>
		    <tr height="20"> 
				<th  align='center'>
...
<%						}
			...
			if(archivo.getComentario()!=null){				
				...
				com = com.replaceAll("[^\\w.,xxxxxxxxxxxx@\\n:;!()&/$%=|-]"," ");
...
<%				if(archivo.getAutorizacion()!=null) if(archivo.getAutorizacion().equals("N")&&!vista.equals("xEstrategia")&&!vista.equals("xMateria"))out.print("<img id='img-"+archivo.getId()+"' src='img/estrella.gif' border='0'/> ");
...
<%				if(vista.equals("xAlumnoEstrategia")||vista.equals("xAlumno")||vista.equals("paraAlumno")||vista.equals("xMateria")||vista.equals("paraAlumnoxEstrategia"))tempNombreAlumnoAnt=tempNombreAlumno;
		    ...
		    <tr>
		    	<td colspan='5'><br>Tamaxo mxximo del archivo: <b>7 Megabytes.</b><br>
