	...
	<tr>
		...
		<td width="631" colspan="2" align="justify" valign="top">
			...
			<tr> 
		  	...
		  	<tr> 
            	<td height="10" colspan="6" style="text-align:center" valign="top">
			    	<h4>BOLETA PARCIAL DE CALIFICACIONES</h4>			  			  
				</td>
          	</tr>
		  	<tr>
            	<td height="10" colspan="6" style="text-align:center" valign="top">		  
          	...
          	<tr> 
            	...
            	<th width="17%" style="text-align:center"><h4>Avance(%)</h4></th>
          	</tr>
<% 		
	...
	if (mapAvanceEvaluaciones.containsKey(alum.getCursoCargaId())){
		avance =  mapAvanceEvaluaciones.get(alum.getCursoCargaId());
	}
	...
	String notaMateria 	= "";
	notaMateria = alum.getNota();
	float notaMaxima = 0;
	String textoMaximo = "<sapan class='badge bg-dark'>"+notaMaxima+"</span>";
	if (alum.getTipoCalId().equals("I")){
		float puntosEvaluados = 0;
		if (mapPuntosEvaluados.containsKey(alum.getCursoCargaId())){
			puntosEvaluados = Float.valueOf(mapPuntosEvaluados.get(alum.getCursoCargaId()));
		}
		
		float puntosGanados = 0;
		if (mapPuntosGanados.containsKey(alum.getCodigoPersonal()+alum.getCursoCargaId())){
			puntosGanados = Float.valueOf(mapPuntosGanados.get(alum.getCodigoPersonal()+alum.getCursoCargaId()));
		}
		float eficiencia = 0;
	...
	if (plan.getEscala().equals("10")){
		notaNumero = notaNumero / 10;
	}
...
<%-- 	    	    <td width="4%" height="10" style="text-align:right"><h5><%=alum.getNota()%></h5></td> --%>
	    	    <td width="4%" height="10" style="text-align:right"><h5><%=notaNumero%></h5></td>
				<td width="5%" height="10" style="text-align:right"><h5><%=alum.getNotaExtra()%></h5></td>
	        	<td width="26%" height="10" style="text-align:center"><h5><%=avance%></h5></td>
			</tr>
<% 	} %>
			<tr> 
