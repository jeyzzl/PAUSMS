...
<div class="alert alert-info">
	<a class="btn btn-primary" href="inscritos"><i class="fas fa-arrow-left"></i></a>
</div>
<table class="table table-bordered">
<%	
	int cont = 0;
	acceso = AccesoU.mapeaRegId(conEnoc, codigoPersonal);
	ArrayList<aca.vista.Inscritos> lisInscritos = inscritosU.getListAllUM(conEnoc," WHERE ENOC.FACULTAD(CARRERA_ID) = '"+facultad+"' ORDER BY ENOC.FACULTAD(CARRERA_ID), CARRERA_ID, APELLIDO_PATERNO,APELLIDO_MATERNO,NOMBRE");
    for(int j=0; j<lisInscritos.size(); j++){			
		aca.vista.Inscritos insc = (aca.vista.Inscritos) lisInscritos.get(j);
			...
			if( acceso.getAccesos().indexOf(insc.getCarreraId())!= -1 || acceso.getAdministrador().equals("S") || acceso.getSupervisor().equals("S") ){				
				if ((cont % 3) == 0 && cont!=0) { %> <tr><td colspan="3">&nbsp;</td></tr><%}
%>
	<td align="center">
	<table class="table table-bordered">
