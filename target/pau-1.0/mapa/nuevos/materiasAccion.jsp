...
<%
		...
		case 7:{//Copiar datos
			...
			if(!mapaNuevoCursoU.updateReg(conEnoc, mapaNuevoCursoDestino)){
				error = true;
			}
			
			ArrayList<MapaNuevoUnidad> listUnidades = mapaNuevoUnidadU.getListCurso(conEnoc, cursoIdOrigen, "ORDER BY UNIDAD_ID");
			...
			for(int i = 0; i < listUnidades.size(); i++){
				...
				if(mapaNuevoUnidadU.insertReg(conEnoc, mapaNuevoUnidad)){
					...
					listBiblioUnidad = mapaNuevoBiblioUnidadU.getListCursoUnidad(conEnoc, cursoIdOrigen, mapaNuevoUnidad.getUnidadId(), "ORDER BY (SELECT TIPO FROM ENOC.MAPA_NUEVO_BIBLIOGRAFIA"+ 
					...
					for(int j = 0; j < listBiblioUnidad.size(); j++){
						...
						mapaNuevoBiblioUnidad.setCursoId(cursoIdDestino);//Reemplazamos el curso
