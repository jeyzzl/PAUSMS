...
<jsp:useBean id="MusiAlumno" scope="page" class="aca.musica.MusiAlumno"/>
...
<%	
	...
	MusiAlumno.setCodigoId((String) session.getAttribute("CodigoId"));
	if (MusiAlumno.existeReg(conEnoc)){
		existeAlumno = true;
	}	
...
<div class="container-fluid">
	<h3> Estado de Cuenta <small> ( <%= codigoId %>] - <%= aca.musica.MusiAlumno.getNombre(conEnoc,codigoId,"NOMBRE")%> ) </small></h3>
	...
	<table class="table table-condensed table-striped">  
  	<tr>
		...
		<th>Referencia</th>
	 	...
	 	<th>Cargo</th>
	 	<th>Crédito</th>
	 	<th><spring:message code="aca.Saldo"/></th>
  	</tr>
<%
	if (strOpcion.equals("1")){
...
<%		
	...
	for (int i=0; i< lisMovtos.size(); i++){
		...
		aca.musica.MusiCalculoDet caldet = new aca.musica.MusiCalculoDet();
		caldet.setCodigoId(codigoId);
		caldet.setPeriodoId(periodoId);
		caldet.setCuentaId(Movtos.getCuentaId());
		if (caldet.existeReg(conEnoc)){
			caldet.mapeaRegId(conEnoc, codigoId, periodoId, Movtos.getCuentaId());
			beca = caldet.getBeca();
		}else{
			beca = "0";
		}
		...
		double becas = cargo*(Double.parseDouble(beca)/100);			
%>
	<tr>
		<td align="center">
	  	  <i title="Editar" class="fas fa-edit" title="Modificar" onclick="location.href='captura?Accion=1&CodigoId=<%=codigoId%>&Folio=<%=Movtos.getFolio()%>';" ></i>
		  <i title="Eliminar" class="fas fa-trash-alt" title="Eliminar" onclick="Borrar('<%= Movtos.getCodigoId() %>','<%=Movtos.getFolio()%>');" ></i>
		</td>
	    <td align="center">	  
		 	<%=i+1%>
		</td>
		...
		<td align="center"><%= getformato.format(saldo+becas)%></td>	  
	</tr>
<%  	
	}
...
%>	
	<tr bgcolor="#b5e1f8">
    	<td align="center"><a class="btn btn-primary" href="captura?Accion=0">Cancelar</a></td>
    	<td align="center">
...
%>
		...
		<td align="center">
	  		<input type="submit" class="btn btn-primary"  name="Grabar" onclick="javascript:Graba()" value="Grabar">
		</td>
  	</tr>  	
<%
		}else{
%>
	<tr><td align="left" colspan="10"><a class="btn btn-primary"  href='javascript:Nuevo()'><spring:message code='aca.Nuevo'/></a></td></tr>
<%		
		}
%>
	</table> 
