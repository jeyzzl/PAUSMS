...
<jsp:useBean id="AlumFoto" scope="page" class="aca.alumno.AlumFotografia"/>
...
<%
	...
	try{
		...
		// Recorta la imagen para resolucion 360*480
    	if (resolucion.equals("1")){
    		aca.util.RecortarImagen.getSubImagen(foto);
    		AlumFoto.setResolucion("1");
    		if (AlumFoto.existeReg(conEnoc)){
    			AlumFoto.updateReg(conEnoc);
    		}else{
    			AlumFoto.insertReg(conEnoc);
    		}
    	}else{    		
    		AlumFoto.setResolucion("2");
    		if (AlumFoto.existeReg(conEnoc)){
    			AlumFoto.updateReg(conEnoc);
    		}else{
    			AlumFoto.insertReg(conEnoc);
		...
		if (AlumFotoUtil.existeReg(conArchivo, matricula, "O")){
			AlumFotoUtil.updateReg(conArchivo,alumFoto);
		}else{
			AlumFotoUtil.insertRegByte(conArchivo,alumFoto);
		}
    }catch(Exception ex){
